...
// 複数取得
export const getItems = (itemsName, orderBy = 'updated', sort = 'desc') => db.collection(itemsName).orderBy(orderBy, sort).get().then((data) => {
  const ary = []
  data.forEach((doc) => {
    ary.push(doc.data())
  })
...
export const setItem = (item, itemsName) =>
  ...
  .then((res = {}) => {
    ...
    if(!_.isEqual(res, item)) {
      return saveEntry(item, itemsName, message)
      .then((data) => Object.assign({}, data, { [isCreated ? 'isCreated' : 'isUpdated']: true }))
    }
...
export const saveEntriesEvents = (items, itemsName) => {
  ...
  return Promise.all(saveEvents).then((values) => {
...
export const removeItems = (items, itemsName, target = 'uuid') =>
