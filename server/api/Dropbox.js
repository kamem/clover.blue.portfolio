      ...
      request(params,
        (err, response, body) => {
          ...
          Promise.all([
            docsFolderInfo,
            docsMetadata
          ]).then(([folderInfoValues, metadataValues]) => {
            const docsPromises = _.chain(folderInfoValues)
            .filter(({ folders }) => _.some(folders, { name: folderName }))
            .reject(({ id }) => _.find(metadataValues, { id }).status['.tag'] === 'deleted')
            .map(({ id, folders }) => {
              const tags = _.chain(folders).reject({ name: folderName }).map('name').value()
